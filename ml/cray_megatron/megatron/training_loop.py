...
class TrainingLoop:
    ...
    def training_step(self, batch):
        """Execute training step based on training mode."""
    ...
    def training_step_language_model(self, batch):
        """Training step for language model (original behavior)."""
    ...
    def training_step_embedding(self, batch):
        """Training step for embedding model."""

        device = self.training_state.model_info["distribution_strategy"]["device"]

        start_time = time.time()

        # Forward pass - prepare sentence features for CoSENTLoss
        # CoSENTLoss expects a list of two dicts: [sentence1_features, sentence2_features]
        sentence1_features = {
            key.replace('sentence1_', ''): batch[key].to(device) 
            for key in batch if key.startswith('sentence1_')
        }
        sentence2_features = {
            key.replace('sentence2_', ''): batch[key].to(device) 
            for key in batch if key.startswith('sentence2_')
        }
