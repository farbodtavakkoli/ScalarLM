...
logger = logging.getLogger(__name__)
 ...
 # Change were made here - Added get_hidden_size function
def get_hidden_size(config):
    """Get hidden size from config, handling different attribute names and nested configs."""
    # Try direct attributes first
    for attr_name in ['hidden_size', 'd_model', 'n_embd', 'dim', 'hidden_dim', 'model_dim']:
        if hasattr(config, attr_name):
            return getattr(config, attr_name)
    
    # Try nested configs (e.g., Gemma3 has text_config and vision_config)
    ...
    # If none found, print available attributes to help debug
    logger.error(f"Available config attributes: {[attr for attr in dir(config) if not attr.startswith('_')]}")
    raise AttributeError(
        f"Cannot find hidden size attribute in {type(config)}. "
        f"Available attributes logged above."
    )
...
class TokenformerMLPAdapter(nn.Module):
    ...
    def forward(self, 
                ...
                *args, # Change made here
                **kwargs): # Change made here
        
        
        all_base_layer_results = self.layer(hidden_states, *args, **kwargs) # Change made here

        tokenformer_results = self.tokenformer_op(hidden_states) # Change made here
...
class TokenformerAttentionAdapter(nn.Module):
    ...
    def forward(self, query, base_layer_results) -> torch.Tensor:
        ...
        # sum the two outputs
        #layer_and_adaptor_sum = base_layer_results + tokenformer_results
        print(f"Base Layer:" , base_layer_results[0].shape) # Change made here
        print(f"TokenFormer:" , tokenformer_results.shape) # Change made here
        layer_and_adaptor_sum = base_layer_results[0] + tokenformer_results # Change made here


        return layer_and_adaptor_sum, base_layer_results[1] # Change made here
...
class TokenformerSurgeon(ABC):
    ...
    def _is_attn_layer(self, layer_name): # Change were made here
        return "attn" in layer_name.split(".")[-1] # Change were made here
    ...
    def update_mlp(self, name, layer):
        ...
        self._recursive_setattr(
            ...
            TokenformerMLPAdapter(
                layer, get_hidden_size(self.model.config), device=self.device  # Change were made here
    ...
    def insert_adapter_modules(self):
        ...
        for name, layer in self.model.named_modules():
            if "vision" in name.lower():  # Change were made here
                continue                  # Change were made here
